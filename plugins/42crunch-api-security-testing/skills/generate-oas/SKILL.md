...
If yes, **prompt the user** for the root directory:
...
If **Postman**, **prompt the user** twice, as two separate prompts:
1. First prompt — ask for the Postman collection file path (JSON, v2.0 or
   v2.1 — required).
2. Second prompt — ask for the Postman environment file path (optional —
   skip if none).

If **Insomnia**, **prompt the user** twice, as two separate prompts:
1. First prompt — ask for the Insomnia export file path (required) — JSON
   ...
   user to specify it.
2. Second prompt — ask for a separate Insomnia environment export file path
...
**Prompt the user** to confirm the output file path, defaulting to:
